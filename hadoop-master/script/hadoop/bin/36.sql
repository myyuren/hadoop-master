use process;
create table hbase361 row format delimited fields terminated by '\t' stored as textfile as 
select 
...
---客户总数
create table hbase_custnum362 as 
...
---合同总金额
create table hbase_contractmoney362 as 
select concat(d.groupid,"-", 
case 
when d.fiveclass = '' 
then "0" 
...
---其他
create table hbase_other362 as 
...
---总统计
create table hbase362 row format delimited fields terminated by '\t' stored as textfile as 
...
from hbase_custnum362 hcn join hbase_contractmoney362 hcm on hcn.key =hcm.key join hbase_other362 ho on hcm.key = ho.key;

drop table hbase_custnum362;
drop table hbase_contractmoney362;
drop table hbase_other362;
