use process;
create table hbase341 row format delimited fields terminated by '\t' stored as textfile as 
select 
...
---客户总数
create table hbase_custnum342 as 
...
---合同总金额
create table hbase_contractmoney342 as 
select concat(d.instCityCode,"-",d.groupid,"-", 
case 
when d.fiveclass = '' 
then "0" 
...
---其他
create table hbase_other342 as 
...
from mlc_main_client c join xcrms_grouprelation r on c.deptcode = r.lowgroupid join
om_organization o on r.upgroupid = o.groupid join grouptype gt on o.groupid =  gt.groupid and gt.orgtype ='XLS' group by c.instCityCode,o.groupid,c.fiveclass;

---总统计
create table hbase342 row format delimited fields terminated by '\t' stored as textfile as 
...
from hbase_custnum342 hcn join hbase_contractmoney342 hcm on hcn.key =hcm.key join hbase_other342 ho on hcm.key = ho.key; 
