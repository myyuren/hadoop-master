...
create table mlc_main_client_ stored as parquet as
SELECT distinct
    mlc.busitype,
    mlc.custid,
    mlc.cname,
...
where ISOUTTABLELOAN ='F' AND mlc.BMCFG!='813' AND mlc.BMCFG !='816') c;
...
select o.groupid, o.name, c.* from 
mlc_main_client c join xcrms_grouprelation r on c.deptcode = r.lowgroupid join
om_organization o on r.upgroupid = o.groupid and o.orglevel = 1 limit 3;

select o.groupid, o.name, c.* from 
mlc_main_client c join xcrms_grouprelation r on c.deptcode = r.lowgroupid join
om_organization o on r.upgroupid = o.groupid and o.orglevel = 3 limit 3;
